...
const HexagonalMessageGrid = () => {
    ...
    const getConversationPositions = () => {
        ...
        for (let i = 0; i < 25; i++) {
            ...
            if (isLeft) {
                ...
                positions.push({q: 1, r: row});     // Right column - same row for proper alignment
            }
        }
    ...
    // Springy pan animation
    useEffect(() => {
        if (!isDragging && (Math.abs(velocity.x) > 0.1 || Math.abs(velocity.y) > 0.1)) {
            const animate = () => {
                setViewState(prev => ({
                    ...prev,
                    x: isLocked ? prev.x : prev.x + velocity.x,
                    y: prev.y + velocity.y
                }));

                setVelocity(prev => ({
                    x: isLocked ? 0 : prev.x * 0.92,
                    y: prev.y * 0.92
                }));

                if ((!isLocked && Math.abs(velocity.x) > 0.1) || Math.abs(velocity.y) > 0.1) {
    ...
    const handleMouseMove = (e) => {
        ...
        if (isLocked) {
            ...
            setVelocity({x: deltaX * 0.3, y: deltaY * 0.3});
        }
    ...
    const handleWheel = (e) => {
        ...
        if (isLocked) {
            ...
            // Calculate new view position to keep the world point under the mouse
    ...
    const backgroundHexes = generateBackgroundHexes();
        ...
        <div
            ...
            <div className="absolute top-6 right-6 z-50 flex flex-col space-y-2">
                ...
                {!isLocked && (
                    <>
                        <button
                            onClick={() => setViewState(prev => ({...prev, zoom: Math.min(3.0, prev.zoom * 1.25)}))}
                            className="glass-control-button"
                        >
                            <Plus className="w-4 h-4"/>
                        </button>
                        <button
                            onClick={() => setViewState(prev => ({...prev, zoom: Math.max(0.4, prev.zoom * 0.8)}))}
                            className="glass-control-button"
                        >
            ...
            >
                ...
                >
                    ...
                    {backgroundHexes.map(({q, r, x, y}) => (
                        <div
                            key={`bg-${q}-${r}`}
                            className="absolute hex-tile hex-empty"
                            style={{
                                left: x - hexSize,
                                top: y - hexSize,
                                width: hexWidth,
                    ...
                    {isTyping && (
                        <div
                            className="absolute animate-fade-in"
                            style={{
                                left: hexToPixel(conversationPositions[messages.length].q, conversationPositions[messages.length].r).x - hexSize,
                                top: hexToPixel(conversationPositions[messages.length].q, conversationPositions[messages.length].r).y - hexSize,
                                width: hexWidth,
                                height: hexHeight,
                            }}
                        >
            ...
            <style jsx>{`
                ...
                .hex-message {
                    ...
                    border: 2px solid rgba(255, 255, 255, 0.1); /* Reduced opacity */
                ...
                .hex-message-user {
                    background: rgba(6, 182, 212, 0.06); /* Reduced opacity */
                    border: 2px solid rgba(6, 182, 212, 0.15); /* Reduced opacity */
                }
